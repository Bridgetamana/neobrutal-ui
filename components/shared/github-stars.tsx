...
export async function GitHubStars({ owner, repo }: { owner: string; repo: string }) {
    ...
    return (
        <a
            ...
            aria-label={`Star ${owner}/${repo} on GitHub — ${stars} stars`}
            className="inline-flex items-center gap-1.5 rounded-base border-2 border-black bg-white px-3 py-1.5 text-sm font-bold transition-all hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none shadow-brutal"
        >
            <svg
                fill="currentColor"
                viewBox="0 0 24 24"
