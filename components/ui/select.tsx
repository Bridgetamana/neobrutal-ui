...
>(({ className, children, ...props }, ref) => (
    <BaseSelect.Trigger
        ref={ref}
        className={cn(
            "flex min-h-11 w-full items-center justify-between rounded-base border-2 border-black bg-white px-3 py-2 text-sm placeholder:text-black focus-brutal disabled:cursor-not-allowed disabled:opacity-50 transition-brutal active:scale-[0.97] [&>span]:line-clamp-1",
            className
        )}
        {...props}
    >
...
>(({ className, children, ...props }, ref) => (
    <BaseSelect.Item
        ref={ref}
        className={cn(
            "relative flex min-h-11 w-full cursor-default select-none items-center rounded-base py-1.5 pl-8 pr-2 text-sm font-bold focus-brutal data-highlighted:bg-main data-highlighted:text-black data-disabled:pointer-events-none data-disabled:opacity-50 text-black",
            className
        )}
        {...props}
    >
